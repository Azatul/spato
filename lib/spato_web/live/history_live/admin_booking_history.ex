defmodule SpatoWeb.HistoryLive.AdminBookingHistory do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> assign(:selected_table, :meeting_room)
     |> assign(:equipment_history, Bookings.list_closed_equipment_bookings(nil))
     |> assign(:room_history, Bookings.list_closed_room_bookings(nil))
     |> assign(:vehicle_history, Bookings.list_closed_vehicle_bookings(nil))
     |> assign(:catering_history, Bookings.list_closed_catering_bookings(nil))
     |> assign(:search_query, "")
  ...
  defp filter_room_history(list, q, status, date_str) do
    ...
    |> filter_by_query(q, fn b ->
      [b.purpose, b.status, (b.meeting_room && b.meeting_room.name) || (b.room && b.room.name), (b.meeting_room && b.meeting_room.location) || (b.room && b.room.location), b.notes]
    end)
  end
        ...
        <main class="flex-1 overflow-y-auto pt-20 p-6 bg-gray-100">
          ...
          <%= if @selected_table == :equipment do %>
            <section class="bg-white p-4 md:p-6 rounded-xl shadow-md">
              ...
              <.table id="admin_equipment_history" rows={@equipment_history_page}>
                ...
                <:col :let={booking} label="Dibuat Oleh">
                  <%= if booking.user do %>
                    <div class="flex flex-col">
                      <span class="font-medium text-gray-900"><%= booking.user.user_profile && booking.user.user_profile.full_name || booking.user.email %></span>
                    </div>
                  <% else %>
                    <span class="text-gray-400">—</span>
                  <% end %>
          ...
          <%= if @selected_table == :meeting_room do %>
            <section class="bg-white p-4 md:p-6 rounded-xl shadow-md">
              ...
              <.table id="admin_room_history" rows={@room_history_page}>
                <:col :let={booking} label="ID"><%= booking.id %></:col>
          ...
          <%= if @selected_table == :vehicle do %>
            <section class="bg-white p-4 md:p-6 rounded-xl shadow-md">
              ...
              <.table id="admin_vehicle_history" rows={@vehicle_history_page}>
                <:col :let={booking} label="ID"><%= booking.id %></:col>
          ...
          <%= if @selected_table == :catering do %>
            <section class="bg-white p-4 md:p-6 rounded-xl shadow-md">
              ...
              <.table id="admin_catering_history" rows={@catering_history_page}>
                <:col :let={booking} label="ID"><%= booking.id %></:col>
