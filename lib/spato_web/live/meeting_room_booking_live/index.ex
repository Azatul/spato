defmodule SpatoWeb.MeetingRoomBookingLive.Index do
  ...
  def handle_event("submit_cancel", %{"reason" => reason}, socket) do
    ...
    case Bookings.cancel_meeting_room_booking(booking, user, reason) do
      {:ok, _} ->
        {:noreply,
         socket
         |> assign(:show_cancel_modal, false)
        ...
        <main class="flex-1 overflow-y-auto pt-20 p-6 transition-all duration-300 bg-gray-100">
          <section class="mb-4">
            ...
            <section class="bg-white p-4 md:p-6 rounded-xl shadow-md">
              ...
              <:action :let={booking}>
                <div class="flex gap-2">
                  <%= if booking.status == "pending" do %>
                    <.link
                      patch={~p"/meeting_room_bookings/#{booking.id}/edit?page=#{@page}&q=#{@search_query}&status=#{@filter_status}&date=#{@filter_date}"}
              ...
              <!-- Cancel modal -->
              <.modal :if={@show_cancel_modal} id="cancel-meeting-room-modal" show on_cancel={JS.push("close_modal")}>
