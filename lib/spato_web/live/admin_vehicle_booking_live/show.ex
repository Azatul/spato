defmodule SpatoWeb.AdminVehicleBookingLive.Show do
  ...
  def render(assigns) do
    ...
    <.header>
      ...
      <:actions>
        <.link patch={~p"/admin/vehicle_bookings/#{@vehicle_booking}"} phx-click={JS.push_focus()}>
    ...
    <.list>
      ...
      <:item title="Rejection reason">{@vehicle_booking.rejection_reason}</:item>
    </.list>

    <.back navigate={~p"/admin/vehicle_bookings"}>Back to vehicle_bookings</.back>

    <.modal :if={@live_action == :edit} id="vehicle_booking-modal" show on_cancel={JS.patch(~p"/admin/vehicle_bookings/#{@vehicle_booking}")}>
      <.live_component
        ...
        patch={~p"/admin/vehicle_bookings/#{@vehicle_booking}"}
      />
    </.modal>
    """
  end
