      ...
      <div class="flex flex-col flex-1">
        ...
        <main class="flex-1 overflow-y-auto pt-20 p-6 transition-all duration-300 bg-gray-100">
          <section class="mb-4">
            ...
            <!-- Middle Section: Add Equipment Booking Button -->
              ...
              <div class="flex flex-wrap items-end gap-4 w-full">
                ...
                <div class="flex-1 min-w-[150px]">
                  <.input
                    field={@form[:query]}
                    label="Carian"
                    placeholder="Cari peralatan, no. siri..."
                    class="w-full"
                  />
                </div>
                  ...
                  <div class="relative mb-3">
                    <img src={equipment.photo_url || "/images/equipment.jpg"} class="w-full h-40 object-cover rounded-lg" />
                      ...
                      <%= case equipment.type do %>
                        <% "laptop" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-green-500">Laptop / Notebook</span>
                        <% "projector" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-yellow-500">Projektor</span>
                        <% "projector_screen" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-purple-500">Projektor Screen</span>
                        <% "printer" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-pink-500">Printer Mudah Alih</span>
                        <% "kamera" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-red-500">Kamera</span>
                        <% "speaker" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-blue-500">Speaker</span>
                        <% "laser_pointer" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-indigo-500">Laser Pointer</span>
                        <% "extension_cord" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-gray-500">Extension Cord</span>
                        <% "whiteboard" -> %>
                          <span class="px-1.5 py-0.5 rounded-full text-white text-xs font-semibold bg-teal-500">Whiteboard / Flipchart</span>
                        <% _ -> %>
